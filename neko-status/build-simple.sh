...
if [ -f "build/neko-status_darwin_arm64" ]; then
    ...
    chmod +x build/neko-status_darwin_universal
    chmod +x build/neko-status_linux_universal
    chmod +x build/neko-status_universal
    ls -la build/
    echo "M1版本可执行文件: build/neko-status_darwin_arm64"
    echo "macOS通用版本: build/neko-status_darwin_universal"
    echo "Linux通用版本: build/neko-status_linux_universal"
    echo "全局通用版本: build/neko-status_universal"
else
    echo "构建失败，请检查错误信息"
fi 