...
(function() {
    ...
    function initSettingsListener() {
        // 监听正确的事件名称 'personalization-settings-updated'
        document.addEventListener('personalization-settings-updated', function(e) {
            if (e.detail) {
                console.log('接收到设置更新事件:', e.detail);

                // 更新缓存
                try {
                    sessionStorage.setItem('personalization-settings', JSON.stringify(e.detail));
                } catch (e) {
                    console.warn('无法更新设置缓存:', e);
                }

                // 应用新设置
                applyWallpaperSettings(e.detail);
                applyCardSettings(e.detail);
            }
        });
        ...
        window.addEventListener('message', function(e) {
            // 验证消息类型
            if (e.data && e.data.type === 'personalization-settings-updated' && e.data.settings) {
                console.log('接收到 postMessage 设置更新:', e.data.settings);

                // 更新缓存
                try {
                    sessionStorage.setItem('personalization-settings', JSON.stringify(e.data.settings));
                } catch (e) {
                    console.warn('无法更新设置缓存:', e);
                }

                // 应用新设置
                applyWallpaperSettings(e.data.settings);
                applyCardSettings(e.data.settings);
            }
        });
        ...
        window.addEventListener('personalization-settings-changed', function(e) {
            if (e.detail && e.detail.settings) {
                console.log('接收到旧格式设置更改事件:', e.detail.settings);
