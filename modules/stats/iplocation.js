...
class IPLocationService {
    ...
    async updateServerLocation(server, db) {
        ...
        try {
            ...
            if (!ip) {
                console.error(`[${new Date().toISOString()}] 服务器无有效IP地址: ${server.name}`);
                ...
                return { success: false, data: serverData, error: '服务器无有效IP地址' };
            }
            
            // 检查更新失败记录
            if (this.updateFailures.has(sid)) {
                const failInfo = this.updateFailures.get(sid);
                
                // 如果重试次数超过最大值，则跳过
                if (failInfo.retries >= this.maxRetries) {
                    console.log(`[${new Date().toISOString()}] 跳过已达最大重试次数的服务器: ${server.name}`);
                    return null;
                }
                
                // 如果上次尝试时间距离现在小于重试间隔的一半，则跳过
                // 这是为了避免频繁重试导致的资源浪费
                const timeSinceLastTry = now - failInfo.lastTry;
                if (timeSinceLastTry < this.retryInterval / 2) {
                    console.log(`[${new Date().toISOString()}] 跳过近期已尝试的服务器: ${server.name}, 上次尝试: ${Math.floor(timeSinceLastTry/1000/60)}分钟前`);
                    return null;
                }
                
                console.log(`[${new Date().toISOString()}] 重试获取服务器位置: ${server.name} (${ip}), 第${failInfo.retries + 1}次尝试`);
            }
            ...
            if (locationData && locationData.countryCode && locationData.countryCode !== '--') {
                // 如果获取到了有效的国家代码，则认为更新成功，即使 locationData.success 为 false
                console.log(`[${new Date().toISOString()}] 获取到有效的国家代码: ${server.name} (${ip}) -> ${locationData.countryCode}`);
            ...
            } else if (!locationData || !locationData.success) {
                const errorMsg = locationData?.error || '未知错误';
                console.error(`[${new Date().toISOString()}] 获取位置信息失败: ${server.name} (${ip}), 错误: ${errorMsg}`);
                ...
                if (!serverData.location.country) {
                    serverData.location.country = {
                        auto_detect: true,
                        auto_detect_failed: true,
                        manual: false
                    };
                } else {
                    serverData.location.country.auto_detect_failed = true;
                }
                
                serverData.location.error = errorMsg;
                serverData.location.lastTry = now;
                
                // 保存到数据库
                db.servers.upd_data(sid, serverData);
                
                return { success: false, data: serverData, error: errorMsg };
            }
            ...
            console.log(`[${new Date().toISOString()}] 更新服务器位置成功: ${server.name} (${locationData.country})`);
            return { success: true, data: serverData };
        } catch (error) {
            console.error(`[${new Date().toISOString()}] 更新服务器位置失败:`, error);
            ...
            return { success: false, data: serverData, error: error.message || '未知错误' };
        }
    }
